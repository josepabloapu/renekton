...
int trackObject(IplImage* imgThresh, int R, int G, int B, int player, CvMoments* moments){
...
int main()
{
	...
	//pong.setFramerateLimit(1);

	//music
	sf::Music bgm;
	bgm.openFromFile("../game5/musica.wav");
	bgm.setPitch(1.5);
	bgm.setLoop(true);
	bgm.play();
...
CvMoments* moments = (CvMoments*)malloc(sizeof(CvMoments));
	...
	imgThresh = GetThresholdedImage(imgHSV, 24, 0, 224, 46, 89, 256);
		...
		cvWaitKey (10);
	     ...
	     player100 = trackObject(imgThresh, 255, 0, 0, 1, moments);
	...
	imgThresh2 = GetThresholdedImage(imgHSV2, 24, 0, 224, 46, 89, 256);
		...
		player200 = trackObject(imgThresh2, 0, 255, 0, 2, moments);
